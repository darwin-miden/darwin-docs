...
| 4 | AggLayer BridgeAsset functional | 🟡 first-party bridge-out CLI ready; runtime demo waits on docker daemon | **What's solid:** `WrappedBasketToken.sol` + 13 Foundry tests; `darwin-bridge-adapter` ships **`darwin_bridge_out`**, a Rust binary that builds + submits a B2AGG note via `miden-agglayer 0.14` + `miden-client 0.14` directly — no dependency on upstream's container-resident `bridge-out-tool`. Same code path swaps to the Miden public testnet bridge once it ships (point `--node-url` + `--bridge-id` at the canonical bridge). **Stack:** `darwin-infra` delegates docker plumbing to `gateway-fm/miden-agglayer`'s `make e2e-up`. Four Darwin glue scripts wire DCC in: `darwin-bridge-up.sh` (delegate), `darwin-bridge-register-dcc.sh` (`forge create WrappedBasketToken` + `admin_registerFaucet`, persists L1 addr + mirror faucet id), `darwin-bridge-out-dcc.sh` (invokes `darwin_bridge_out` then watches bridge-service for `ready_for_claim`), `darwin-bridge-down.sh`. **Remaining:** start docker daemon + run the four scripts. Upstream flags `miden-node v0.14.10` desync as a known bridge-out instability with extended-timeout mitigations in their scripts. |
