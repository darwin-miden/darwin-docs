...
| 4 | AggLayer BridgeAsset functional | 🟡 partial — solid L1 + Rust types, no on-chain bridge tx | **What's solid:** `WrappedBasketToken.sol` ships with 13 Foundry tests covering the full bridge-only mint/burn surface. `darwin-bridge-adapter::{b2agg, claim, eth}` ship typed Rust builders (no stubs). **What's not yet done:** the compose file at `darwin-infra/compose/docker-compose.yml` is a skeleton — services listed and image tags pinned to real public registries (hermeznetwork/zkevm-bridge-service:v0.6.0, 0xpolygon/agglayer:latest), but missing env-vars and the L1 contract bootstrap (PolygonZkEVMBridge / GlobalExitRootManager need to be deployed on Anvil before bridge-service can start). `agglayer-up.sh` clones the external repos but `docker compose up` will fail on missing config until the bootstrap is written. Estimated 1–2 days of integration work to land a real B2AGG round-trip on the local stack, then bridge-admin coordination for a public testnet round-trip per spec §10.5. **No real on-chain bridge transaction has been executed.** |
