...
| 5 | Flow A end-to-end on testnet | 🟡 (both halves shipped, atomic combine ready) | Mint half + deposit half demonstrated as separate txs. **Real-bodies controller deployed on testnet**: account `0x171f46fecf1bca8005ae068a8dfe77`, deploy tx `0xff0a85ad…a1a9fb90`, code commitment `0x5d65429d…b61db1d2`. `compute_nav` / `compute_mint_amount` / `compute_redeem_amount` run real `miden::core::math::u64::div` on-chain. **Atomic DepositNote** assembles into a `miden_protocol::NoteScript` via `darwin::math` and the math executes correctly under miden-vm 0.22 (`100 * 9970 / 10000 = 99` validated). See `tests/atomic_deposit_note.rs`. Remaining: a Rust binary that wraps the note in `miden_client::Note` + `Client::new_transaction()` to submit on-chain — pure plumbing. |
