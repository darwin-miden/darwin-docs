...
| 4 | AggLayer BridgeAsset functional | 🟡 Darwin glue ready; runtime demo waits on docker daemon | **What's solid:** `WrappedBasketToken.sol` ships with 13 Foundry tests; `darwin-bridge-adapter::{b2agg, claim, eth}` ship typed Rust builders + a `local_stack_round_trip.rs` integration test (`--ignored`-gated). **Stack:** rather than duplicate Polygon/Miden upstream, `darwin-infra` delegates to [`gateway-fm/miden-agglayer`](https://github.com/gateway-fm/miden-agglayer)'s `make e2e-up` — upstream ships the canonical `docker-compose.e2e.yml` (with healthchecks + ordering), `replay-txs.sh` (bootstraps PolygonZkEVMBridge + GlobalExitRootManager on Anvil), `bridge-out-tool`, and `e2e-l1-to-l2.sh` + `e2e-l2-to-l1.sh`. Four Darwin glue scripts wire DCC in: `darwin-bridge-up.sh` (delegate), `darwin-bridge-register-dcc.sh` (`forge create WrappedBasketToken` on Anvil + `admin_registerFaucet`), `darwin-bridge-out-dcc.sh` (drives upstream's `e2e-l2-to-l1.sh`), `darwin-bridge-down.sh`. **Remaining:** start docker daemon + run the four scripts to observe a real Miden→L1 bridge-out. Upstream flags `miden-node v0.14.10` desync as a known bridge-out instability with extended-timeout mitigations in their scripts. |
